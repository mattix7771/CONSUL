...
"""
Collaborative Legislation comments summaries

This script generates for each legislation project a summary of all its comments.
...
def check_file(file_name):
    if os.path.isfile(file_name):
        ...
        try:
            logging.info('Missing file in legislation comments summaries: ' + str(file_name))
        except NameError:
            print('No logging')
...
# Output files:
comments_summaries_filename = 'ml_comments_summaries_legislation.json'
comments_summaries_filename_csv = 'ml_comments_summaries_legislation.csv'
...
data_path = '../data'
config_file = 'legislation_summary_comments_textrank.ini'
logging_file ='legislation_summary_comments_textrank.log'
...
comments_input_df = comments_input_df[comments_input_df['commentable_type'] == 'Legislation::Annotation']
...
if len(comments_input_df) == 0:
    logging.info('No Legislation comments found to summarise.')
    with open(os.path.join(data_path,comments_summaries_filename), 'w') as file:
        file.write('[]')
    os._exit(0)
...
# # Stats previously commented out
...
comments_df['commentable_type'] = ['Legislation::Annotation']*len(comments_df)
